...
%%
function [cost,msll,beamw,eccen,GD]= costs_lite1000(pattern,mask2,SU,SV,pbi,Lag_wt,BW_def,BPinfo)
% alpha=0.5;beta=0.5;gamma=0.0;psi=10;%20
...
%% ALternative beamwidth computation
...
beamw=bd1;%sqrt(bd1.^2+bd2.^2);% NOTE: This is overall BW
msll=(pm2-pm);
% eccen=sqrt(max(ap)^2-min(ap)^2)/max(ap);
eccen=sqrt(bd1^2-bd2^2)/bd1;
GD=gd(db2mag(pattern+pbi),SU,SV);
cost=ov_cost_single(beamw,msll,eccen,GD,Lag_wt,BW_def,BPinfo);% eccen=scale_fac;
end