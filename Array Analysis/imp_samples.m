function posa=imp_samples(Nt,Rc_u)
...
%      FU=2*xg(ids).*sqrt(1-rg(ids));FV=2*yg(ids).*sqrt(1-rg(ids));
     ...
     if 1
     a=2*pi*(1-sqrt(1-Rc_u))/Nt;d=sqrt(a);n=1;
     M_lat=round(asin(sqrt(Rc_u))/d);
     d_lat=asin(sqrt(Rc_u))/M_lat;
     d_lon=a/d_lat;
     for i=1:M_lat
        lat_a=(i-0.5)*asin(sqrt(Rc_u))/M_lat;
        M_lon=round(2*pi*sin(lat_a)/d_lon);
        nn=n+M_lon-1;
        FU(n:nn)=sin(lat_a)*cos(((1:M_lon)-0.5)*2*pi/M_lon);
        FV(n:nn)=sin(lat_a)*sin(((1:M_lon)-0.5)*2*pi/M_lon);
        n=nn+1;
     end
     else
     %%
     FU=sqrt(Rc_u)*2*(rand(1,Nt)-0.5);
     FV=sqrt(Rc_u)*2*(rand(1,Nt)-0.5);
     end
     Fw=ones(size(FU));
end
