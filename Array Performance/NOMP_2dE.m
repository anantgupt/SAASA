function [omegaList, gainList, residueList] = NOMP_2dE(y,Phi,txf,tyf, ...
			      	   K_est,  R_s , R_c,posa)
% SUMMARY:
% 
...
while true
    ...
    % Note: omega_new = [omega_1 ; omega_2];
    [omega_new, gain_new, y_r] = ...
        detectNew(y_r, Phi, ant_idx_1, ant_idx_2,posa);
    
    if (VERBOSE) fprintf('New sinusoid detected: [%.3f ; %.3f]\n' , omega_new(1), omega_new(2)); end;
    
    % Stopping rule:
    if length(gainList) >= K_est
        break;
    end    
    ...
    for i = 1:R_s
        [omega_new, gain_new, y_r] = refineOne(y_r, Phi, omega_new, ...
            gain_new, ant_idx_1, ant_idx_2 , true);
    end
    
    if (VERBOSE) fprintf('New sinusoid refined: [%.3f ; %.3f]\n' , omega_new(1), omega_new(2)); end;
